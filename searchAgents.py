...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        """
        vNode = []
        return (self.startingPosition, vNode)
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                if (nextx, nexty) in self.corners and (nextx, nexty) not in foundCorners:
                    vNode = foundCorners + [(nextx, nexty)]
                    successors.append((((nextx, nexty), vNode), action, 1))
                else:
...
def cornersHeuristic(state, problem):
    ...
    for corner in corners:
        if not corner in vNode:
            unvNode.append(corner)
    while unvNode:
        distance, corner = min([(util.manhattanDistance(node, corner), corner) \
                                for corner in unvNode])
        heuristic += distance
        node = corner
        unvNode.remove(corner)
    return heuristic
